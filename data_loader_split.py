...
def load_data_split(basedir, scene, split, skip=1, try_load_min_depth=True, only_img_files=False):
    ...
    if split == "train":
        nobg_files = find_files('{}/rgb_nobg'.format(split_dir), exts=['*.png', '*.jpg'])
        if len(nobg_files) > 0:
            logger.info('raw img_files: {}'.format(len(nobg_files)))
            nobg_files = nobg_files[::skip]
            assert (len(nobg_files) == cam_cnt)
        else:
            nobg_files = [None, ] * cam_cnt
    else:
        nobg_files = [None, ] * cam_cnt
