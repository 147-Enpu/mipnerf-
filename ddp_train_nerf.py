...
def render_single_image(rank, world_size, models, ray_sampler, chunk_size):
    ...
    #ray_batch = ray_sampler.random_sample(1024, center_crop=False)
    ...
    for s in range(len(ray_batch_split['ray_d'])):
        ...
        for m in range(models['cascade_level']):
            ...
            if m == 0:
                ...
                # background depth
                bg_depth = torch.linspace(0., 1., N_samples + 1).view(
                    [1, ] * len(dots_sh) + [N_samples + 1, ]).expand(dots_sh + [N_samples + 1, ]).to(rank)
                bg_depth = perturb_samples(bg_depth)  # random perturbation during training
                # print("bg_depth:",bg_depth.shape)
                # bg_depth: torch.Size([1024, 65])

                # print("ray_batch['ray_o']:",ray_batch['ray_o'].shape)
                # print("N_samples:",N_samples)
                # print("dots_sh:",dots_sh)
                # print("fg_far_depth:",fg_far_depth.unsqueeze(-1).shape)
                far = fg_far_depth.unsqueeze(-1)
                near = fg_near_depth.unsqueeze(-1)
                # radii = ray_batch['radii'].unsqueeze(-1)
                fg_depth, (fg_mean, fg_var) = sample_along_rays(ray_o, ray_d, radii, N_samples,
                                                                near, far, randomized=True, lindisp=False,
                                                                ray_shape="cone")

                # bg_mean, bg_var = cast_rays(bg_depth, ray_batch['ray_o'], ray_batch['ray_d'], ray_batch['radii'], ray_shape="cone")

                # print("t_vals:",t_vals.shape)
                # print("mean:",mean.shape)
                # print("var:", var.shape)
                # t_vals: torch.Size([1024, 65])
                # mean: torch.Size([1024, 64, 3])
                # var: torch.Size([1024, 64, 3])
                del fg_far_depth
                del fg_near_depth
                del far
                del near
                ...
                # print("fg_weights:",fg_weights.shape)

                fg_depth, (fg_mean, fg_var) = resample_along_rays(ray_o, ray_d, radii,
                                                                  fg_depth.to(rank),
                                                                  fg_weights, randomized=True,
            ...
            with torch.no_grad():
                ret = net(ray_o, ray_d, fg_depth, bg_depth, fg_mean, fg_var)
    ...
    if rank == 0:
        ...
        for m in range(len(ret_merge_chunk)):
            for key in ret_merge_chunk[m]:
                ...
                # print("ret_merge_rank[m][key]:",ret_merge_rank[m][key][0].shape)
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        net = MipNeRF(args, use_viewdirs=args.use_viewdirs,
...
def ddp_train_nerf(rank, args):
    ...
    what_train_to_log = 0
    lossmult = torch.ones_like(ray_samplers[0].random_sample(args.N_rand, center_crop=False)['ray_o'][..., :1]).to(rank)
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            N_samples_bg = models['cascade_samples'][m]
            if m == 0:
                # foreground depth
                ...
                # background depth
                bg_depth = torch.linspace(0., 1., N_samples_bg+1).view(
                            [1, ] * len(dots_sh) + [N_samples_bg+1,]).expand(dots_sh + [N_samples_bg+1,]).to(rank)
                bg_depth = perturb_samples(bg_depth)   # random perturbation during training
                # print("bg_depth:",bg_depth.shape)
                # bg_depth: torch.Size([1024, 65])
                ...
                near = fg_near_depth.unsqueeze(-1)
                #radii = ray_batch['radii'].unsqueeze(-1)
                fg_depth, (fg_mean, fg_var) = sample_along_rays(ray_batch['ray_o'], ray_batch['ray_d'], ray_batch['radii'], N_samples,
                                                        near, far, randomized=True, lindisp=False,
                                                        ray_shape="cone")
                ...
                bg_weights = bg_weights[..., 1:-1]                              # [..., N_samples-2]
                bg_depth_samples = sample_pdf(bins=bg_depth_mid, weights=bg_weights,
                                              N_samples=N_samples_bg, det=False)    # [..., N_samples]
                bg_depth, _ = torch.sort(torch.cat((bg_depth, bg_depth_samples), dim=-1))

                #print("fg_weights:",fg_weights.shape)

                fg_depth, (fg_mean, fg_var) = resample_along_rays(ray_batch['ray_o'], ray_batch['ray_d'], ray_batch['radii'],
                                                          fg_depth.to(rank),
                                                          fg_weights, randomized=True,
                                                          stop_grad=True, resample_padding=0.01,
                                                          ray_shape="cone")
            ...
            optim.zero_grad()
            ret = net(ray_batch['ray_o'], ray_batch['ray_d'], fg_depth, bg_depth, fg_mean, fg_var)
            all_rets.append(ret)

            rgb_gt = ray_batch['rgb'].to(rank)
            #rgb_nbgt = ray_batch['rgb_nb'].to(rank)
            if 'autoexpo' in ret:
                ...
                # #loss = img2mse(ret['fg_rgb'], rgb_nbgt)
                loss = rgb_loss
                # loss, psnr = loss_func(ret['rgb'], rgb_gt, lossmult)
